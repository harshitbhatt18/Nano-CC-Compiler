...
extern int scopeCount;  // Defined in lexicalAnalyzer.l

// Define the global variables here
int functionCount = 0;
// scopeCount is defined in lexicalAnalyzer.l
...
// Remove the duplicate makeList function - it's already defined in semantic.h
...
int main(int argc, char **argv) {
	...
	sourceCode = (char *)malloc((strlen(argv[1]) + 1) * sizeof(char));
	if (!sourceCode) {
	...
	if (!errorFlag && !semanticErr && arrayIndexErr != 1) {
		...
		FILE *writeParsed = fopen("parsedTable", "w");
		fprintf(writeParsed, "%-16s | %-12s | %-8s\n", "Lexeme", "Type", "Line No");

		for (tokenList *ptr = parsedPtr; ptr != NULL; ptr = ptr->next) {
			fprintf(writeParsed, "%-16s | %-12s | %-8d\n", ptr->token, ptr->type, atoi(ptr->line)-1);
		}
		fclose(writeParsed);

		FILE *writeSymbol = fopen("symbolTable", "w");
		fprintf(writeSymbol, "%-16s | %-12s | %-8s | %-8s | %-8s\n", "Lexeme", "Type", "Line No", "Scope", "Func No");

		for (tokenList *ptr = symbolPtr; ptr != NULL; ptr = ptr->next) {
			fprintf(writeSymbol, "%-16s | %-12s | %-8d | %-8s | %-8d\n", ptr->token, ptr->type, atoi(ptr->line)-1, ptr->scope, ptr->funcCount + 1);
		}
		fclose(writeSymbol);

		FILE *writeConstant = fopen("constantTable", "w");
		fprintf(writeConstant, "%-16s | %-8s\n", "Value", "Line No");

		for (tokenList *ptr = constantPtr; ptr != NULL; ptr = ptr->next) {
			fprintf(writeConstant, "%-16s | %-8d\n", ptr->token, atoi(ptr->line)-1);
		}
